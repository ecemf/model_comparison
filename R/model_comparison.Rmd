...
```{r load_data, include=FALSE}
  ...
  tmp$Period <- 2020
  tmp$Scenario <- "CEDS2019"
  histData <- rbind(histData, tmp)

  # filter
  histData <- histData[histData$Scenario %in% c("CEDS","CEDS2019","EU_ReferenceScenario_2020","James_IMF"),]
...
policyScen <- c("DIAG-Base", "DIAG-NPI", "DIAG-C80-gr5", "DIAG-C0to80-gr5", "DIAG-C400-lin", "DIAG-NZero")
...
linePlot <- function(data, scen=NULL, title="", colorDim="Model", linetypeDim=NULL, xdim="Period", ydim="Value", facetDim="Scenario", legend = list(nRow=NULL), min=0, max=NULL, minPeriod=2005, maxPeriod=2050, labels=NULL){
  ...
  gg <- ggplot(plotData,aes_string(x=xdim ,y=ydim, color=colorDim ,linetype=linetypeDim)) +
    ...
    {if((!(nrow(hist)==0)) & !is.null(linetypeDim)) geom_line(data=histFull,size=1,aes_string(color = colorDim),alpha = 0.5,lty="11") } +
    {if((!(nrow(hist)==0)) & is.null(linetypeDim)) geom_line(data=histFull,size=1,aes(color = source),alpha = 0.5,lty="11") } +
    {if(!(nrow(hist)==0)) geom_text(data=histLabel, aes(label = source,color = source), hjust ="inward", vjust=1.5, alpha = 0.5) } +
    geom_line(size=1) +
    geom_point() +
    facet_wrap(facetDim,drop = FALSE, nrow = 2) +
    theme_singleColumn() +
    theme(
...
barPlot <- function(data, scen=NULL,total=NULL,period=c(2020,2030,2050),title="", legend = list(nRow=NULL), min=0, max=NULL, labels=NULL, pattern=NULL){
  ...
  total <- total[(total$Period %in% period),]
  total$Model <- factor(total$Model, level = levels(plotData$Model))
  #total$Model <- factor(total$Model)
  ...
  #     filter(Period == max(Period))
  #   tmp$Model <- paste0(tmp$Period, " ", tmp$Scenario)
  #   tmp$Period <- 2020
  #   histFull <- NULL
  #   for(sc in scen){
  #     tmp$Scenario <- sc
  ...
  #   }
  #   histFull$Scenario <- factor(histFull$Scenario , levels = scen)
  #   #histFull$Model <- factor(histFull$Model, levels = c(unique(histFull$Model),levels(plotData$Model)))
  # }

  if(!(is.null(labels))){
      fullVars <- levels(data$Variable)
      names(fullVars) <- labels
      labelsVars <- fullVars[unique(data$Variable)]
  }

  if(!(is.null(pattern))){
      fullVars <- levels(data$Variable)
      names(fullVars) <- pattern
      patternVars <- fullVars[unique(data$Variable)]
  }

  gg <- ggplot(plotData,aes(x=Model,y=Value,fill=Variable,pattern=Variable)) +
    geom_hline(yintercept=0, color = "black", size=1, alpha = 0.15) +
    {if(nrow(total)>0)geom_point(data=total,aes(x=Model,y=Value,fill="gray"), shape=23, alpha = 0.5)} +
    {if(!(nrow(hist)==0)) geom_bar(data=histFull,stat='identity')} +
    {if(is.null(pattern)) geom_bar(stat='identity')} +
    {if(!is.null(pattern)) geom_bar_pattern(stat='identity', pattern_key_scale_factor = 0.3, pattern_density=0.5, pattern_spacing = 0.03, pattern_colour = "white", pattern_fill = "white", pattern_alpha=0.4)} +
    facet_wrap(~Scenario+Period, scales='free_x', nrow=2, labeller = labeller(Scenario = function(Variable, Value) {c("", "DIAG-Base","","","DIAG-NPI","","","DIAG-C80-gr5","","", "DIAG-C0to80-gr5","","","DIAG-C400-lin","","","DIAG-NZero","")}),drop = FALSE) +
    theme_singleColumn() +
...
filterData <- function (data, plotName, l = plotList) {
  ...
  out <- do.call(rbind,
      lapply(unique(data$Model), function(model){
         return(data[data$Model == model & data$Variable %in% loopList(lt=l[[plotName]]$vars, varList=vars, dd=data[data$Model == model,]),])
      })
    )
...
plotList <- fromJSON(file="plots.json")
...
g <-
  lapply(names(plotList),function(plot){
    ...
    if(plotList[[plot]]$type == "line"){
      d <- filterData(data=df[df$Scenario!="CEDS2019",],plotName=plot) # filtering data for plot
      p <- do.call(linePlot, c(list(data=d,scen=policyScen),plotList[[plot]]$arg))
      ...
      p <- do.call(barPlot, c(list(data=d,scen=policyScen,total=totalSum),plotList[[plot]]$arg))
    }
    return(p)
  })
