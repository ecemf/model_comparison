...
createModelChangesChartsReport <- function(data, histData, outputFile, outputFolder, saveCharts=TRUE){
  
  version.colors <- setNames(brewer.pal(length(unique(data$version)), "Set3"),unique(data$version))
  ...
  chart <- function(d, h, var){
    
    o <- list()
    o[["var"]] <- var
    
    currData <- d %>% filter(variable == var) 
    currh <- h %>% filter(variable == var) %>% mutate(version = "none")
    ...
    o[["chart"]] <- ggplot(data = rbind(currData,currh), aes(x = period, y = value, color = model)) +
      ...
      geom_line(data=currData, aes( color = version), linewidth = 1) +
      ...
      scale_color_manual(unique(currData$model),
                         values = c(h.colors, version.colors),
                         breaks = unique(d$version),
      ...
      ggtitle(var) + #ggtitle(paste0(var, "/n", unique(currData$model))) +
  ...
  createCharts <- function(dd, hh, tree, reg){
        ...
        lapply(setNames(names(t),names(t)), function(name){ 
          if(is.list(t[[name]])){
            ...
            lapply(setNames(names(t[[name]]),names(t[[name]])), function(varName){
              if(nrow(ddd) == 0 || nrow(data %>% filter(variable == t[[name]][[varName]], model == unique(ddd$model))) == 0){
                print(paste0("Missing data for: ", t[[name]][[varName]]))
                return(list("var" = t[[name]][[varName]],
                            "chart" = paste0("Missing data for: ", t[[name]][[varName]]))
                )
              }
              print(paste0("Creating ", unique(ddd$model), " chart for: ", t[[name]][[varName]]))
              g <- chart(ddd, hhh, t[[name]][[varName]])
              if(saveCharts) {
                ggsave(paste0(outputFolder, "/modelChanges/svg/", cleanFileName(unique(ddd$region)), "/", unique(ddd$model), "/", gsub("\\/", "-", gsub("\\/ ", "-", gsub("\\|", "_", t[[name]][[varName]]))), ".svg"), g$chart, device="svg", width = 12, height = 6, dpi=100, units = "in")
                ggsave(paste0(outputFolder, "/modelChanges/png/", cleanFileName(unique(ddd$region)), "/", unique(ddd$model), "/", gsub("\\/", "-", gsub("\\/ ", "-", gsub("\\|", "_", t[[name]][[varName]]))), ".png"), g$chart, device="png", width = 12, height = 6, dpi=100, units = "in")
              }
              return(g)
            })
          }
    ...
    dir.create(paste0(outputFolder, "/modelChanges/png"), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(outputFolder, "/modelChanges/svg"), recursive = TRUE, showWarnings = FALSE)
    o <- list()
    for(r in reg){
      dir.create(paste0(outputFolder, "/modelChanges/png/", cleanFileName(r)), recursive = TRUE, showWarnings = FALSE)
      dir.create(paste0(outputFolder, "/modelChanges/svg/", cleanFileName(r)), recursive = TRUE, showWarnings = FALSE)
      for(m in unique(dd$model)){
        if(!(file.exists(paste0(outputFolder,"/modelChanges/", cleanFileName(r), "_modelChanges_", m, ".rds")))){
          dir.create(paste0(outputFolder, "/modelChanges/png/", cleanFileName(r), "/", m), recursive = TRUE, showWarnings = FALSE)
          dir.create(paste0(outputFolder, "/modelChanges/svg/", cleanFileName(r), "/", m), recursive = TRUE, showWarnings = FALSE)
          o[[r]][[m]] <- recurseTree(dd %>% filter(region %in% r, model %in% m), hh %>% filter(region %in% r), tree)
          print(paste0("Saving ", cleanFileName(r), " charts for: ", m))
          saveRDS(o[[r]][[m]],paste0(outputFolder,"/modelChanges/", cleanFileName(r), "_modelChanges_", m, ".rds"))
        }
      }
    }
    return(o)
  ...
  # creating charts
  out <- createCharts(data,hist,outVars,unique(data$region))
  
  # create html reports
  print(paste0("Creating model changes charts html report for scenario: ", unique(data$scenario), ", regions: ", toString(cleanFileName(unique(data$region))), ", and models: ", toString(unique(data$model))))
  rmarkdown::render("./reports/modelChanges.Rmd", output_file = outputFile, 
                    params = list(
                      regions = cleanFileName(unique(data$region)), 
                      scenario = unique(data$scenario), 
                      models = unique(data$model),
                      dataFolder = paste0(".",outputFolder)))
  #paste0(".", outputFolder, "/model_changes - ", unique(data$scenario), " - " , toString(cleanFileName(unique(data$region))), ".html")
}
